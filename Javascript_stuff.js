    ...
    class ChangelogManager {
        constructor() {
            ...
            this.currentVersion = "v2.0.1"; // <-- Itt változtasd a verziót
            ...
            return [
                {
                    ...
                    title: "Frissítés",
                    changes: [
                        "➕ Új: A napra kattintva mostmár lehet megjegyzést írni az adott naphoz.",
                        "🔧 Javítás: Pünkösd Vasárnap mostmár ünnepnapnak számít.",
                        "🔧 Javítás: A 25 év alatti adókedvezmény mostmár látható a bérszámfejtésben a SZJA levonásnál(Eddig is jól számolta csak vizuálisan nem mutatta jól).",
                        "✨ Fejlesztés: A hónapok közötti váltás mostmár szinkronban, egyszerre változik a Naptár és a Bérszámfejtés menüben.",
                        "✨ Fejlesztés: A súgó menü frissítése a program használatáról és részletes leírás képletekkel és magyarázatokkal, hogy a bérszámfejtés mi alapján történik"
                    ]
                },
            ];
        }
            ...
            try {
                ...
                switch (label) {
                    ...
                    case "Nettó": {
                        ...
                        const szja = this.calculateMonthlyValue("Rendszeres SZJA előleg", monthIndex, year); // Ez már tartalmazza a levonást
                        
                        // A 25 év alatti kedvezményt már nem adjuk hozzá külön, mert az SZJA-ban van kezelve
                        return Math.round(bruttoBer + otherIncome + childBenefit - tbJarulek - szja - restaurantEntry);
                    }                
        ...
        generateCalendar() {
            try {
                ...
                while (currentDate <= daysInMonth) {
                    ...
                    const savedNote = this.yearlyData[this.currentYear].notes?.[this.currentMonth]?.[currentDate] || "";
                    if (savedNote) {
                    ...
                    const existingNote = this.yearlyData[this.currentYear].notes?.[this.currentMonth]?.[clickedDay] || "";
                    ...
                    const saveNote = () => {
                        ...
                        if (noteText) {
                            this.yearlyData[this.currentYear].notes[this.currentMonth][clickedDay] = noteText;
                        } else {
                            // Ha üres a megjegyzés, töröljük
                            delete this.yearlyData[this.currentYear].notes[this.currentMonth][clickedDay];
                        }
